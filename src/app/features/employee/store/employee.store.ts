...
export class EmployeeStore {
    ...
    private readonly filteredList = computed(() => {
        ...
        return raw.filter(e => {
            const matchesName = e.username.toLowerCase().includes(s.searchTermName.toLowerCase()) ||
    ...
    readonly totalPages = computed(() => Math.ceil(this.filteredList().length / this.state().pageSize));
